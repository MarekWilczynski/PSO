...
class LungsSegmentationMatlab(MatlabSegmentationFunction):
    ...
    def get_result(self, parameters_vector):
        ...
        se_1 = self._to_matlab_double(list(se_1[:,0]))
        se_2 = self._to_matlab_double(list(se_2[:,0]))
        se_3 = self._to_matlab_double(list(se_3[:,0]))
        se_4 = self._to_matlab_double(list(se_4[:,0]))
        se_5 = self._to_matlab_double(list(se_5[:,0]))

        input_converted = self._input_image        

        binary_image = self._segmentation(input_converted, self._spacing, threshhold, object_to_remove_size, se_1, se_2, se_3, se_4, se_5)
        return binary_image